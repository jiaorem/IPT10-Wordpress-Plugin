<?php 
if ( ! function_exists( 'videography_filmmaker_related_posts_function' ) ) {
	function videography_filmmaker_related_posts_function() {
		wp_reset_postdata();
		global $post;
		...
		$args = array(
			...
			'posts_per_page'    => absint( get_theme_mod( 'videography_filmmaker_related_post_count', '3' ) ),
		);
		// Related by categories
		if ( get_theme_mod( 'videography_filmmaker_post_order', 'categories' ) == 'categories' ) {
		...
		// Related by tags
		if ( get_theme_mod( 'videography_filmmaker_post_order', 'categories' ) == 'tags' ) {
...
$related_posts = videography_filmmaker_related_posts_function(); ?>

<?php if ( $related_posts->have_posts() ): ?>

	<div class="related-posts clearfix py-3">
		<?php if ( get_theme_mod('videography_filmmaker_related_posts_title','Related Posts') != '' ) {?>
			<h2 class="related-posts-main-title"><?php echo esc_html( get_theme_mod('videography_filmmaker_related_posts_title',__('Related Posts','videography-filmmaker')) ); ?></h2>
		<?php }?>
		<div class="row">
